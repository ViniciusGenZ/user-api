...
async function create(input: IRoleCreateRequest): Promise<IRoleCreateResponse> {
...
async function update(
	id_role: number,
	input: IRoleUpdateRequest,
): Promise<IRoleUpdateResponse> {
	return (
		await repository.update({ id_role }, { ...input, updated_at: new Date() })
	).raw[0];
}
...
async function del({
	id_role,
	by,
}: IRoleDeleteRequest): Promise<IRoleDeleteResponse> {
	return (
